...
from open_notebook.utils.version_utils import (
    ...
    get_package_json_version_from_github_async,
)
...
# Repository checked for new releases. The app version lives in
# frontend/package.json; "HEAD" resolves to the repo's default branch.
VERSION_CHECK_REPO_URL = os.getenv(
    "VERSION_CHECK_REPO_URL",
    "https://github.com/nghoainam1902ps4-lgtm/ChatBot5400-Version02",
)
VERSION_CHECK_REF = os.getenv("VERSION_CHECK_REF", "HEAD")

_REPO_ROOT = Path(__file__).parent.parent.parent


def get_version() -> str:
    """Read the app version from frontend/package.json (fallback: pyproject.toml)"""
    try:
        with open(_REPO_ROOT / "frontend" / "package.json", encoding="utf-8") as f:
            version = json.load(f).get("version")
            if version:
                return version
    except Exception as e:
        logger.warning(f"Could not read version from frontend/package.json: {e}")
...
async def get_latest_version_cached(current_version: str) -> tuple[Optional[str], bool]:
    ...
    try:
        ...
        latest_version = await get_package_json_version_from_github_async(
            VERSION_CHECK_REPO_URL, VERSION_CHECK_REF
        )
        ...
        # Compare versions
        has_update = compare_app_versions(current_version, latest_version) < 0
